...
type MapTypeForLegend = Exclude<MapType, "none">;

type Props = {
  mapType: MapTypeForLegend;
};

export default function MapLegend({ mapType }: Props) {
  const data = mapTypeData[mapType];
  const title = mapTypeData[mapType].title;
  const stops = mapTypeData[mapType].stops;
  ...
  const gradientStops = stops
    .map(
      (object) =>
        `${object.color} ${normalizeValue(object.value, minValue, maxValue)}%`,
    )
    .join(",");
  ...
  return (
    <div className="absolute top-4 right-4 z-1000 w-96 xs:w-96 rounded-xl shadow-lg p-4 bg-background/50 border border-accent/70 flex flex-col gap-4">
...
function normalizeValue(x: number, xmin: number, xmax: number): number {
  // to prevent infinite value
  if (xmin === xmax) {
    return 0;
  }
...
const mapTypeData: Record<
  string,
  { title: string; unit: string; stops: NonEmptyArray<ColorStop> }
> = {
  precipitation_new: {
    title: "Rain (mm)",
    unit: "mm",
